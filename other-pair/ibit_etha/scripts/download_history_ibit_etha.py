#!/usr/bin/env python3
"""
Robust IBKR historical downloader for IBIT + ETHA (1h bars).
...
Outputs (default):
  other-pair/ibit_etha/data/raw/{symbol}_1h_rth.parquet
  other-pair/ibit_etha/data/raw/{symbol}_1h_rth.csv
"""
...
def build_symbol_state(
    ...
    log(f"{symbol} requesting head timestamp...")
    head_raw = ib_await(
        ib,
        ib.reqHeadTimeStampAsync(contract, whatToShow=what_to_show, useRTH=use_rth, formatDate=2),
        timeout_s=startup_timeout_s,
        label=f"{symbol} reqHeadTimeStamp",
    )
    head_utc = parse_head_timestamp(head_raw)
...
def main() -> None:
    ...
    ap.add_argument("--symbols", nargs="+", default=["IBIT", "ETHA"])
    ...
    ap.add_argument("--use-rth", action="store_true", default=True)
    ...
    for sym in symbols:
        ...
        log(f"{sym} qualifying contract...")
        q = ib_await(
            ib,
            ib.qualifyContractsAsync(c),
            timeout_s=args.startup_timeout_seconds,
            label=f"{sym} qualifyContracts",
        )
        if q:
            c = q[0]
        contracts[sym] = c
        log(
            f"{sym} qualified conId={getattr(c, 'conId', None)} "
            f"exchange={getattr(c, 'exchange', None)} primaryExch={getattr(c, 'primaryExchange', None)}"
        )
